...
import { IndicatorsQuery, retrieveIndicators } from "../retrieve-indicators";
import { Indicators } from "../../../models/indicators.model";
import { IndicatorGateway } from "../../../ports/indicator.gateway";
...
class StubIndicatorGateway implements IndicatorGateway {
  ...
  async retrieveIndicators(query: IndicatorsQuery): Promise<Indicators> {
    return this._indicators.find(
      ({ query: q1 }) =>
        q1.symbol === query.symbol &&
        q1.timeUnit === query.timeUnit &&
        q1.startDate === query.startDate,
    )!.indicators;
  }
