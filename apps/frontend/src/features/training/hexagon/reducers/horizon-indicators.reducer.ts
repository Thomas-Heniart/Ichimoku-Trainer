...
import { ONE_DAY_IN_MS } from '../../constants.ts'
...
const horizonIndicatorsSlice = createSlice({
    name: 'indicators/horizon',
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(retrieveAlarmIndicators.fulfilled, (_, { payload: { indicators } }) => {
            if (indicators) return indicators.horizon
        ...
        builder.addCase(loadNextInterventionCandle.fulfilled, (state, { payload: { candle } }) => {
            ...
            const candleTimestampAsHour = startOfDay(new UTCDate(candle.openTime))
            if (lastCandleTimestamp.valueOf() < candleTimestampAsHour.valueOf()) {
                state.timestamps.push(state.timestamps[state.timestamps.length - 1] + ONE_DAY_IN_MS)
...
export const horizonIndicatorsReducer = horizonIndicatorsSlice.reducer
